...
const PAYMENT_COLUMNS = [
  ...
  { label: "Amount (TSH)", key: "amount", icon: "💰" },
  { label: "Balance (TSH)", key: "balance", icon: "💳" },
];
            ...
            <div className="text-sm">
              {type === "commission" && (
                <span className="font-medium text-gray-700 dark:text-gray-300">
                  Total Net Commission: TSH {allData.reduce((sum, item) => sum + (parseFloat(item.net) || 0), 0).toLocaleString()}
                </span>
              )}
              {type === "payment" && allData.length > 0 && (
                <span className="font-medium text-gray-700 dark:text-gray-300">
                  Current Balance: TSH {formatCurrency(allData[allData.length - 1]?.balance)}
                </span>
              )}
            </div>
